...
// Marker used for cheap optional detection in the flat finalize hot path.
// This avoids `Mirror(reflecting:)` while still catching `Optional<Wrapped>` values behind `Any`.
private protocol _DecodeAnyOptional {}

extension Optional: _DecodeAnyOptional {}
...
extension Qs {
  ...
  ///   background queue.
  nonisolated public static func decodeAsync(
    _ input: Any?,
    options: DecodeOptions = .init(),
  ...
  nonisolated internal static func decodeAsyncBoxed(
    ...
    try await withCheckedThrowingContinuation { cont in
      DispatchQueue.global(qos: qos.value)
        .async {
          do {
  ...
  private static func canSkipFinalizeForFlatObject(_ object: [String: Any]) -> Bool {
    ...
    while let node = stack.popLast() {
      if node is Undefined || node is _DecodeAnyOptional { return false }
