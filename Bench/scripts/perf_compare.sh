...
for run in $(seq 1 "$runs"); do
  ...
  LC_ALL=C "$BIN" "$scenario_cmd" >"$snapshot_file"
...
for key, items in sorted(groups.items(), key=lambda x: x[0]):
    ...
    if kind == "encode":
        ...
        _, runtime, name, count, comma, utf8, out_len, key_count = key
        case = {
            "kind": "decode",
            "runtime": runtime,
            "name": name,
...
for case in cases:
    if case["kind"] == "encode":
        ...
        print(
            ...
            f"len={case['len']:4d} keys={case['keys']:4d}  ms={case['ms_per_op_median']:.3f}"
        )
    ...
    for case in cases:
        ...
        if case["kind"] == "encode":
            ...
            print(
                f"  decode {case['runtime']:5s} {case['name']} count={case['count']:4d} "
                f"comma={str(case['comma']).lower():5s} utf8={str(case['utf8']).lower():5s} "
                f"len={case['len']:4d} keys={case['keys']:4d}  ms={delta:+.2f}%"
            )
PY
